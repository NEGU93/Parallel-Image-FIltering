...
int get_everything_together(animated_gif * image) {
	...
	*/
	...
	for (i = 0; i < n_images; i++) { 	// as each image can have different sizes, 
										//I make sure to have the larger for a buffer
		width = image->width[i];
		height = image->height[i];
		count = width * height * sizeof(pixel);
		max_count = MAX(max_count, count);
	}
	temp_buf = (pixel **)malloc( max_count * sizeof(pixel *) );
	...
	if (me == ROOT) {
		...
		#endif
		for (i = 0; i < n_images; i++) { // Have to eliminate the ones computed by myself
			if( (i % P) == ROOT) { 
				#ifdef GATHER_DEBUG
				fprintf(stderr, "Image %d computed by root\n", i);
				#endif
				continue; 
			} // Don't have to take anything
			// wait for sth
			MPI_Recv(*temp_buf, max_count, MPI_BYTE, MPI_ANY_SOURCE, MPI_ANY_TAG, MPI_COMM_WORLD, &status);
			#ifdef GATHER_DEBUG
			fprintf(stderr, "Image %d received from %d, noy copying it...\n", status.MPI_TAG, status.MPI_SOURCE);
			#endif
			width = image->width[status.MPI_TAG];
			height = image->height[status.MPI_TAG];
			count = width * height * sizeof(pixel);				
			memcpy(*(p + status.MPI_TAG), *temp_buf, count);
		}
	}
	else {
